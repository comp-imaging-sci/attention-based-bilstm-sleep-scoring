        ...
        for fname in fnames:
            ...
            for epoch in range(num_epochs):
                with tf.io.TFRecordWriter(os.path.join(des_path, os.path.splitext(os.path.basename(fname))[0] + f"_epoch{epoch}.tfrecords")) as writer:
                    ...
                    epoch_data = gcamp[:,:,:151,epoch]
                    # z-score standardization
                    epoch_data = standardization(epoch_data, mask)
                    # min-max
                    #epoch_data = min_max_normalization(epoch_data, mask)
...
def parse_tfrecord_fn(example):
    ...
    feature_description = {
            'features': tf.io.FixedLenFeature([128, 128, 168], tf.float32), 
...
if __name__ == '__main__':
    ...
    des_path = '/shared/planck/MRI/xiaohui/mouse_optical/sleep_stage/2022-Ben-9s-raw-masked-tempzscore-broadband-states-tfrecords'
    ...
    #dataset = tf.data.TFRecordDataset('/shared/planck/MRI/xiaohui/mouse_optical/sleep_stage/2022-Ben-10s-raw-masked-tempzscore-broadband-states-tfrecords/191030-M5-fc1-GSR_Ben_epoch0.tfrecords')
